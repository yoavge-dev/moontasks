...
function style(t: string) { return STYLES[t as SourceType] ?? STYLES.hardcoded; }
...
export function CMSSpecifier() {
  ...
  const analyze = useCallback(async (file: File) => {
    ...
    reader.onload = async (e) => {
      ...
      try {
        ...
        if (json.data?.annotations) setAnnotations(json.data.annotations);
        else setError(json.error ?? "No annotations returned.");
      } catch (err) {
  ...
  const exportMarkdown = () => {
    const rows = annotations.map((a) => `| ${a.label} | ${a.sourceType} | ${a.fieldPath} | ${a.fieldType} |`).join("\n");
    const md = ["# CMS Field Specification", "", "| Element | Source | Field Path | Type |", "|---------|--------|------------|------|", rows].join("\n");
    ...
    <div className="flex overflow-hidden -mx-6 -mt-6" style={{ height: "calc(100vh - 3rem)" }}>
      <div className="w-[320px] shrink-0 flex flex-col border-r bg-background overflow-hidden">
        <div className="px-4 py-3.5 border-b shrink-0">
          <h2 className="text-sm font-semibold">CMS Specifier <span className="text-[10px] text-muted-foreground font-normal">(AI)</span></h2>
          <p className="text-[11px] text-muted-foreground mt-0.5">Upload a screenshot — fields detected automatically</p>
        </div>
        <div className="px-4 py-2 border-b flex gap-4 text-[10px] font-semibold shrink-0">
          {(["section", "provider", "hardcoded"] as SourceType[]).map((t) => (
            <span key={t} className="flex items-center gap-1.5"><span className={cn("w-2 h-2 rounded-full", style(t).dot)} />{t}</span>
          ))}
        </div>
        <div className="flex-1 overflow-y-auto">
          {isLoading && <div className="flex flex-col items-center justify-center h-full gap-3 text-muted-foreground"><Loader2 className="h-6 w-6 animate-spin" /><p className="text-sm">Analyzing…</p></div>}
          {!isLoading && error && <div className="p-4"><p className="text-sm text-destructive">{error}</p></div>}
          {!isLoading && !error && annotations.length === 0 && (
            <div className="flex flex-col items-center justify-center h-full gap-3 text-muted-foreground p-6 text-center">
              ...
              <p className="text-sm">Upload a screenshot and the AI will map every element to your CMS schema.</p>
            </div>
          )}
            ...
            return (
              <div key={ann.id} className="px-4 py-3 border-b last:border-0 space-y-1">
                <div className="flex items-center gap-2">
                  <span className={cn("h-4 w-4 rounded-full text-white text-[9px] font-bold flex items-center justify-center shrink-0", s.badge)}>{i + 1}</span>
                  <span className="text-xs font-semibold truncate flex-1">{ann.label}</span>
                  <span className={cn("text-[9px] font-semibold px-1.5 py-0.5 rounded-full shrink-0", s.pill)}>{ann.sourceType}</span>
                </div>
                <div className="pl-6">
      ...
      <div className="flex-1 flex flex-col overflow-hidden bg-muted/20">
        {!imageObjectUrl ? (
          <div
            className={cn("flex-1 flex flex-col items-center justify-center gap-4 cursor-pointer transition-colors", isDragging && "bg-primary/5")}
            onDragOver={(e) => { e.preventDefault(); setIsDragging(true); }}
            onDragLeave={() => setIsDragging(false)}
            onDrop={(e) => { e.preventDefault(); setIsDragging(false); const f = e.dataTransfer.files[0]; if (f) analyze(f); }}
            onClick={() => fileInputRef.current?.click()}
          >
            <input ref={fileInputRef} type="file" accept="image/*" className="hidden" onChange={(e) => { const f = e.target.files?.[0]; if (f) analyze(f); }} />
            <div className="flex flex-col items-center gap-3 text-muted-foreground">
              ...
              <div className="text-center">
                ...
                <p className="text-xs mt-0.5">AI maps all fields instantly using your CMS schema</p>
              </div>
            </div>
          </div>
        ) : (
          <div className="flex-1 overflow-auto flex items-start justify-center p-6 relative">
            <button onClick={reset} className="absolute top-4 right-4 z-10 flex items-center gap-1.5 text-[11px] bg-background border rounded-full px-2.5 py-1 shadow-sm hover:bg-muted transition-colors">
              <RefreshCw className="h-3 w-3" /> New screenshot
            </button>
            <div className="relative inline-block">
              <img src={imageObjectUrl} alt="Screenshot" className="rounded-lg border shadow-sm max-h-[85vh] max-w-full block" draggable={false} />
              {isLoading && (
                <div className="absolute inset-0 bg-background/60 rounded-lg flex items-center justify-center">
                  <div className="flex flex-col items-center gap-2 text-sm text-muted-foreground">
                    <Loader2 className="h-6 w-6 animate-spin" /><span>Detecting fields…</span>
                  </div>
                </div>
              )}
              {annotations.map((ann) => {
                ...
                return (
                  <div key={ann.id} className={cn("absolute border-2 rounded pointer-events-none", s.box)}
                    ...
                    <span className={cn("absolute -top-5 left-0 text-[9px] font-bold px-1.5 py-0.5 rounded text-white whitespace-nowrap max-w-[150px] overflow-hidden text-ellipsis block", s.badge)}>
                      {ann.label}
                    </span>
                  </div>
                );
